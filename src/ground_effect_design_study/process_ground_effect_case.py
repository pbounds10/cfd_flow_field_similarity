...
from design_similarity.cluster_model_training import train_models
from design_similarity.localization import cluster_all_meshes_radially
from design_similarity.similarity_scores import calculate_offset_distance
...
def import_raw_data():
    cfd_data_raw = pd.read_parquet(
        os.path.join(
            "/mnt/d/ground_effect_processed_data_python",
            "ground_effect_study_raw_data_gold_11092025.parquet",
        )
    )
...
def make_cell_sets(
    ...
    regionalized_data = cfd_data_raw.copy()
    if selected_cases is not None:
        selected_cases.append(focus_case)
        regionalized_data = regionalized_data[
            regionalized_data["case_id"].isin(selected_cases)
        ]
...
def normalize_targets(regionalized_data: pd.DataFrame):
    ...
    data_to_cluster[FEATURES_TRANS] = StandardScaler().fit_transform(
        data_to_cluster[features]
    )
...
def median_normalize_velocity(
    ...
    focus_data_only["u_feature_mag"] = np.sqrt(
        np.sum(focus_data_only[feature_names] ** 2, axis=1)
    )

    focus_data_median = focus_data_only.groupby("coarse_mesh_parent_cell_id")[
        "u_feature_mag"
    ].median()
    ...
    return data_to_cluster[feature_names] / np.tile(
        data_to_cluster["u_feature_mag"].values.reshape(-1, 1), (1, 2)
    )
...
def _predict_one_cluster(cell_set):
    ...
    model = deepcopy(
        trained_models[np.where(trained_models[:, 0] == cell_set[0])[0][0], 1]
    )
...
def _predict_one_offset_cluster(
    ...
    feature_name_map = {
        new_name: old_name
        for new_name, old_name in zip(feature_names_offset, feature_names)
    }
...
def make_offset_cluster_predictions(
    ...
    with multiprocessing.pool.Pool(12) as pool:
        all_case_clusters_separate = pool.starmap(
            _predict_one_offset_cluster,
            zip(grouped_data, repeat(feature_names), repeat(trained_models)),
        )
...
def save_cluster_prediction_data(
    ...
    for ii, cluster_chunk in enumerate(all_case_clusters_separate):
        cluster_chunk.to_parquet(
            os.path.join(
                "/mnt/d/ground_effect_processed_data_python/clustered_chunks",
    ...
    for ii, cluster_chunk in enumerate(all_case_clusters_separate):
        all_cluster_predictions_joined = pd.concat(
            [all_cluster_predictions_joined, cluster_chunk], axis=0
        )
...
if __name__ == "__main__":
    # This is where you want to save you predictions
    OUTPUT_DIRECTORY = make_output_folder(
        "/mnt/d/ground_effect_processed_data_python/processed_predictions",
        "wig_predictions",
    )
    ...
    # SELECTED_CASES = None
    FOCUS_CASE = "naca_8440_ground_effect_1"
    ...
    data_to_cluster = regionalized_data[
        [
            "case_id",
            "cell_id",
    ...
    data_to_cluster[feature_names] = median_normalize_velocity(
        data_to_cluster, feature_names=["U_0", "U_1"]
    )
    ...
    export_feature_names = (
        ["case_id", "cell_id", "coarse_mesh_parent_cell_id", "C_0", "C_1", "y_wall"]
        + ["U_0_norm", "U_1_norm"]
    ...
    all_case_clusters_separate = make_cluster_predictions(
        data_to_cluster[
            ["coarse_mesh_parent_cell_id", "case_id", "cell_id"] + FEATURES_TRANS
        ],
    )

    print("REGULAR Data Clustered. Now Saving")

    all_cluster_predictions_joined = save_cluster_prediction_data(
        all_case_clusters_separate
    )

    all_cluster_predictions_joined.to_parquet(
        os.path.join(OUTPUT_DIRECTORY, f"normal_all_clusters.parquet")
    )

    print("STARTING OFFSET")
    all_case_clusters_separate = make_offset_cluster_predictions(
        data_to_cluster[
            ["coarse_mesh_parent_cell_id", "case_id", "cell_id"] + offset_feature_names
        ],
    ...
    all_cluster_predictions_joined.to_parquet(
        os.path.join(OUTPUT_DIRECTORY, f"offset_all_clusters.parquet")
    )
